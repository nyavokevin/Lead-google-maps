...
def _website_situation_and_pitch(mode: str, category: str, lead: dict) -> tuple[str, str]:
    """
    ...
    """
    features = FEATURES.get(category, FEATURES["generic"])
...
def _build_system_prompt(category: str, mode: str, lead: dict) -> str:
    """Build the system prompt tailored to the business category and website status."""
    ...
    situation, pitch = _website_situation_and_pitch(mode, category, lead)
...
def _build_user_message(category: str, mode: str, lead: dict) -> str:
    business_name = lead.get("title") or lead.get("name", "cette entreprise")
...
def _build_subject_prompt(category: str, mode: str, lead: dict) -> str:
    business_name = lead.get("title") or lead.get("name", "this business")

    if mode == "reparation_site":
        return f"Votre site web semble inaccessible — proposition de solution pour {business_name}"
...
def generate_email_for_lead(lead: dict, model_name: str) -> dict:
    ...
    chat = _client.Chat(_build_system_prompt(category, mode, lead))
    chat.add_user_message(_build_user_message(category, mode, lead))

    body = _clean_response(_client.respond(chat, model=model_name))
    subject = _build_subject_prompt(category, mode, lead)
